...
const updateTesvik = async (req, res) => {
  try {
    ...
    // Bu yüzden eski veri de aynı türetmeden geçirilerek fotoğraflanır.
    try {
      tesvik.updateMaliHesaplamalar();
    } catch (e) {
    ...
    if (degisikenAlanlar.length > 0) {
      ...
      // (sebep + not), asıl düzenleme sonra ikinci bir kayıt üretiyordu. Son kayıt boş bir
      // başlatma kaydıysa yeni satır açmak yerine onun içi doldurulur; sebep ve not korunur.
...
const updateTesvikDurum = async (req, res) => {
  try {
    ...
    // 🔧 FIX: eski/eksik kayıtlarda zorunlu alan boş olsa bile durum değişebilsin
    // (save() tüm dokümanı doğruladığı için ilgisiz alan durumu bloke ediyordu)
...
const bulkUpdateDurum = async (req, res) => {
  try {
    ...
    // Toplu işlemde kapanmış/iptal edilmiş belgeler korunur (müşteri endişesi)
...
// seçimi her okumada revizyon geçmişinden türetilen durumla eziliyordu.
...
module.exports = {
  ...
  // 🆕 Makine Revizyon Başlat - Atomic $push
    ...
    try {
      ...
      if (Array.isArray(yerli) || Array.isArray(ithal)) {
        ...
        const mapRow = (r, idx) => ({
          ...
          birimFiyatiTl: nz(r.birimFiyatiTl), birimFiyatiFob: nz(r.birimFiyatiFob),
          toplamTutariTl: nz(r.toplamTutariTl || r.toplamTl),
          gumrukDovizKodu: str(r.gumrukDovizKodu || r.doviz).toUpperCase(),
          ...
          kdvMuafiyeti: str(r.kdvMuafiyeti), gumrukVergisiMuafiyeti: str(r.gumrukVergisiMuafiyeti),
          ...
          etuysSecili: !!r.etuysSecili, silinmeTarihi: r.silinmeTarihi,
        ...
        const { rowIdleriKoru } = require('../utils/rowIdKoru');
        const mevcutListeler = await YeniTesvik.findById(id).select('makineListeleri').lean();
        const yerliMapped = Array.isArray(yerli) ? rowIdleriKoru(yerli, mevcutListeler?.makineListeleri?.yerli).satirlar.map(mapRow) : undefined;
        const ithalMapped = Array.isArray(ithal) ? rowIdleriKoru(ithal, mevcutListeler?.makineListeleri?.ithal).satirlar.map(mapRow) : undefined;
  ...
  listMakineNotlari: async (req, res) => {
    try {
      const tesvik = await YeniTesvik.findById(req.params.id).select('makineNotlari').lean();
      if (!tesvik) return res.status(404).json({ success: false, message: 'Teşvik bulunamadı' });
