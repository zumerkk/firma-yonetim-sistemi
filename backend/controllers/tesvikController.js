...
module.exports = {
  ...
  exportSistemExcelCiktisi
};

// 📊 SİSTEM EXCEL ÇIKTISI - Kapsamlı Belge Detayları (CSV Formatında)
async function exportSistemExcelCiktisi(req, res) {
  try {
    const { id } = req.params;
    const { 
    ...
    console.log(`📊 Sistem Excel çıktısı başlatılıyor: ${id}`);
    
    // Tesvik modelini dinamik olarak import et
    const Tesvik = require('../models/Tesvik');
    ...
    if (id.match(/^[0-9a-fA-F]{24}$/)) {
      // ObjectId format (24 karakter hex)
      tesvik = await Tesvik.findById(id)
        .populate('firma', 'tamUnvan firmaId vergiNoTC firmaIl firmaCaddesi firmaIlce')
        .populate('revizyonlar.yapanKullanici', 'adSoyad email rol')
        .populate('olusturanKullanici', 'adSoyad email')
        .populate('sonGuncelleyen', 'adSoyad email')
        .lean();
    ...
    // Stil tanımlamaları - CSV formatına uygun
    const headerStyle = {
    ...
    // 💎 ULTRA ELİT VERİ STİLLERİ
    const dataStyle = {
      font: { 
        color: { argb: 'FF374151' }, // Profesyonel koyu gri
        size: 9,  // Kompakt ve okunabilir
        name: 'Calibri'
      },
      alignment: { 
        horizontal: 'left', 
    ...
    // 💰 ELİT FİNANSAL VERİ STİLİ
    const financialDataStyle = {
      font: { 
        color: { argb: 'FF065F46' }, // Elite koyu yeşil
        size: 9,
        name: 'Calibri',
        bold: true
      },
      ...
      numFmt: '#,##0 ₺', // Daha temiz para formatı (ondalık yok)
      fill: { 
        type: 'pattern', 
        pattern: 'solid', 
        fgColor: { argb: 'FFF0FDF4' } // Çok hafif yeşil arka plan
      }
    };
    
    // Ana sayfa - CSV formatına uygun kolonlar
    const mainSheet = workbook.addWorksheet('Sistem Excel Çıktısı');
    
    // 🏢 PROFESYONEL KURUMSAL TASARIM - Premium İş Raporu Stili
    
    // 🏆 ULTRA ELİT KURUMSAL TASARIM - Fortune 500 Standartları
    // 🎨 HER KATEGORİ İÇİN FARKLI PROFESYONEL RENK PALETİ
    const categoryStyles = {
      'KÜNYE BİLGLERİ': {
        font: { bold: true, color: { argb: 'FFFFFFFF' }, size: 12, name: 'Calibri' },
        fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF8B5CF6' } }, // 💜 Elite Mor
        alignment: { horizontal: 'center', vertical: 'middle', wrapText: true },
        border: { top: { style: 'medium', color: { argb: 'FF7C3AED' } }, left: { style: 'medium', color: { argb: 'FF7C3AED' } }, bottom: { style: 'medium', color: { argb: 'FF7C3AED' } }, right: { style: 'medium', color: { argb: 'FF7C3AED' } } }
      },
      'YATIRIM İLE İLGİLİ BİLGİLER': {
        font: { bold: true, color: { argb: 'FFFFFFFF' }, size: 12, name: 'Calibri' },
        fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF0EA5E9' } }, // 🔵 Profesyonel Mavi
        alignment: { horizontal: 'center', vertical: 'middle', wrapText: true },
        border: { top: { style: 'medium', color: { argb: 'FF0284C7' } }, left: { style: 'medium', color: { argb: 'FF0284C7' } }, bottom: { style: 'medium', color: { argb: 'FF0284C7' } }, right: { style: 'medium', color: { argb: 'FF0284C7' } } }
      },
      'İSTİHDAM': {
        font: { bold: true, color: { argb: 'FFFFFFFF' }, size: 12, name: 'Calibri' },
        fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF10B981' } }, // 🟢 İş Yeşili
        alignment: { horizontal: 'center', vertical: 'middle', wrapText: true },
        border: { top: { style: 'medium', color: { argb: 'FF059669' } }, left: { style: 'medium', color: { argb: 'FF059669' } }, bottom: { style: 'medium', color: { argb: 'FF059669' } }, right: { style: 'medium', color: { argb: 'FF059669' } } }
      },
      'ÜRÜN BİLGLERİ': {
        font: { bold: true, color: { argb: 'FFFFFFFF' }, size: 12, name: 'Calibri' },
        fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF59E0B' } }, // 🟠 Kurumsal Turuncu
        alignment: { horizontal: 'center', vertical: 'middle', wrapText: true },
        border: { top: { style: 'medium', color: { argb: 'FFD97706' } }, left: { style: 'medium', color: { argb: 'FFD97706' } }, bottom: { style: 'medium', color: { argb: 'FFD97706' } }, right: { style: 'medium', color: { argb: 'FFD97706' } } }
      },
      'DESTEK UNSURLARI': {
        font: { bold: true, color: { argb: 'FFFFFFFF' }, size: 12, name: 'Calibri' },
        fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFEF4444' } }, // 🔴 Dikkat Kırmızısı
        alignment: { horizontal: 'center', vertical: 'middle', wrapText: true },
        border: { top: { style: 'medium', color: { argb: 'FFDC2626' } }, left: { style: 'medium', color: { argb: 'FFDC2626' } }, bottom: { style: 'medium', color: { argb: 'FFDC2626' } }, right: { style: 'medium', color: { argb: 'FFDC2626' } } }
      },
      'ÖZEL ŞARTLAR': {
        font: { bold: true, color: { argb: 'FFFFFFFF' }, size: 12, name: 'Calibri' },
        fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF6366F1' } }, // 🟣 İndigo
        alignment: { horizontal: 'center', vertical: 'middle', wrapText: true },
        border: { top: { style: 'medium', color: { argb: 'FF4F46E5' } }, left: { style: 'medium', color: { argb: 'FF4F46E5' } }, bottom: { style: 'medium', color: { argb: 'FF4F46E5' } }, right: { style: 'medium', color: { argb: 'FF4F46E5' } } }
      },
      'FİNANSAL BİLGLER': {
        font: { bold: true, color: { argb: 'FFFFFFFF' }, size: 12, name: 'Calibri' },
        fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF059669' } }, // 💚 Para Yeşili
        alignment: { horizontal: 'center', vertical: 'middle', wrapText: true },
        border: { top: { style: 'medium', color: { argb: 'FF047857' } }, left: { style: 'medium', color: { argb: 'FF047857' } }, bottom: { style: 'medium', color: { argb: 'FF047857' } }, right: { style: 'medium', color: { argb: 'FF047857' } } }
      }
    };
    ...
    const subCategoryStyle = {
      font: { 
        bold: true, 
        color: { argb: 'FF0F243E' }, // Koyu lacivert yazı
        size: 11,  // Hiyerarşik punto
        name: 'Calibri'
      },
      fill: { 
        type: 'pattern', 
        pattern: 'solid', 
        fgColor: { argb: 'FFBDD7EE' } // Microsoft mavi tonu
      },
      alignment: { 
        horizontal: 'center', 
        vertical: 'middle', 
    ...
    const specialCategoryStyle = {
      font: { 
        bold: true, 
        color: { argb: 'FF974806' }, // Daha dengeli altın
        size: 10,  // Tutarlı punto
        name: 'Calibri'
      },
      fill: { 
        type: 'pattern', 
        pattern: 'solid', 
        fgColor: { argb: 'FFFDE9D9' } // Yumuşak turuncu ton
      },
      alignment: { 
        horizontal: 'center', 
        vertical: 'middle', 
      ...
      border: { 
        top: { style: 'thin', color: { argb: 'FFD99694' } }, 
        left: { style: 'thin', color: { argb: 'FFD99694' } }, 
        bottom: { style: 'thin', color: { argb: 'FFD99694' } }, 
        right: { style: 'thin', color: { argb: 'FFD99694' } } 
      }
    };
    ...
    const columnHeaderStyle = {
      font: { 
        bold: true, 
        color: { argb: 'FF1F2937' }, // Daha net siyah
        size: 9,   // Küçük ve net
        name: 'Calibri'
      },
      fill: { 
        type: 'pattern', 
        pattern: 'solid', 
        fgColor: { argb: 'FFF3F4F6' } // Çok hafif gri
      },
      alignment: { 
        horizontal: 'center', 
        vertical: 'middle', 
    ...
    // 🏗️ SATIR 1: MANTIKLI HİYERAŞİK ANA KATEGORİLER
    const mainCategories = [
      // 1️⃣ KÜNYE BİLGLERİ (17 sütun) - YATIRIMCI (6) + BELGE BİLGLERİ (11)
      'KÜNYE BİLGLERİ', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
      // 2️⃣ YATIRIM İLE İLGİLİ BİLGİLER (17 sütun)
      'YATIRIM İLE İLGİLİ BİLGİLER', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
      // 3️⃣ İSTİHDAM (2 sütun)
      'İSTİHDAM', '',
      // 4️⃣ ÜRÜN BİLGİLERİ (54 sütun) - Ayrı bölüm
      'ÜRÜN BİLGİLERİ', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
      // 5️⃣ DESTEK UNSURLARI (16 sütün) - Ayrı bölüm
      'DESTEK UNSURLARI', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
      // 6️⃣ ÖZEL ŞARTLAR (28 sütun) - Ayrı bölüm
      'ÖZEL ŞARTLAR', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
      // 7️⃣ FİNANSAL BİLGİLER (27 sütun) - REVİZE TARİHİ dahil
      'FİNANSAL BİLGİLER', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''
    ];

    // 🎯 SATIR 2: MANTIKLI ALT KATEGORİLER
    const subCategories = [
      // KÜNYE ALT KATEGORİLERİ: YATIRIMCI (6) + BELGE BİLGLERİ (11) = 17 sütun
      'YATIRIMCI', '', '', '', '', '', 'BELGE BİLGLERİ', '', '', '', '', '', '', '', '', '', '',
      // YATIRIM (17) 
      'YATIRIM İLE İLGİLİ BİLGİLER', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
      // İSTİHDAM (2)
      'İSTİHDAM', '',
    ...
    // SATIR 3: Özel kategoriler (sadece finansal kısım detaylı)
    const specialCategories = [
      // KÜNYE (6) + BELGE (11) + YATIRIM (17) + İSTİHDAM (2) = 36 sütun
      '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
      // ÜRÜN (54)
      '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
      // DESTEK (16)
      ...
      // ÖZEL ŞART (28)
      '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '',
      // FİNANSAL (27) - Detaylı alt kategoriler
      'ARAZİ-ARSA BEDELİ', '', '', '',
    ...
    // SATIR 4: Gerçek kolon başlıkları (CSV'deki tam liste)
    const columnHeaders = [
      // KÜNYE BİLGİLERİ (6)
      'GM ID', 'TALEP/SONUÇ', 'REVIZE ID', 'FIRMA ID', 'YATIRIMCI UNVAN', 'SGK SİCİL NO',
      // BELGE BİLGLERİ (11)
      'BELGE ID', 'BELGE NO', 'BELGE TARIHI', 'BELGE MURACAAT TARIHI', 'MÜRACAAT SAYISI', 'BELGE BASLAMA TARIHI', 'BELGE BITIS TARIHI', 'SÜRE UZATIM TARİHİ', 'ÖZELLİKLİ YATIRIM İSE', 'DAYANDIĞI KANUN', 'BELGE DURUMU',
      // YATIRIM İLE İLGİLİ BİLGİLER (17)
      '2-YATIRIM KONUSU', '3-CINSI(1)', '3-CINSI(2)', '3-CINSI(3)', '3-CINSI(4)', 'DESTEK SINIFI', 'YERI IL', 'YERI ILCE', 'ADA', 'PARSEL', 'YATIRIM ADRESI(1)', 'YATIRIM ADRESI(2)', 'YATIRIM ADRESI(3)', 'OSB ISE MUDURLUK', 'İL BAZLI BÖLGE', 'İLÇE BAZLI BÖLGE', 'SERBEST BÖLGE',
      // İSTİHDAM (2)
      'Mevcut Kişi', 'İlave Kişi',
      // ÜRÜN BİLGİLERİ (54 = 9 ürün x 6 alan)
      ...
      // DESTEK UNSURLARI (16 = 8 destek x 2 alan)
      'Destek Unusrları(1)', 'Şartları(1)', 'Destek Unusrları(2)', 'Şartları(2)', 'Destek Unusrları(3)', 'Şartları(3)', 'Destek Unusrları(4)', 'Şartları(4)', 'Destek Unusrları(5)', 'Şartları(5)', 'Destek Unusrları(6)', 'Şartları(6)', 'Destek Unusrları(7)', 'Şartları(7)', 'Destek Unusrları(8)', 'Şartları(8)',
      // ÖZEL ŞARTLAR (28 = 14 şart x 2 alan)
      'Özel Şart Kısaltma 1', 'Özelşart Notu 1', 'Özel Şart Kısaltma 2', 'Özelşart Notu 2', 'Özel Şart Kısaltma 3', 'Özelşart Notu 3', 'Özel Şart Kısaltma 4', 'Özelşart Notu 4', 'Özel Şart Kısaltma 5', 'Özelşart Notu 5', 'Özel Şart Kısaltma 6', 'Özelşart Notu 6', 'Özel Şart Kısaltma 7', 'Özelşart Notu 7', 'Özel Şart Kısaltma 8', 'Özelşart Notu 8', 'Özel Şart Kısaltma 9', 'Özelşart Notu 9', 'Özel Şart Kısaltma 10', 'Özelşart Notu 10', 'Özel Şart Kısaltma 11', 'Özelşart Notu 11', 'Özel Şart Kısaltma 12', 'Özelşart Notu 12', 'Özel Şart Kısaltma 13', 'Özelşart Notu 13', 'Özel Şart Kısaltma 14', 'Özelşart Notu 14',
      // FİNANSAL BİLGİLER (27)
      'Arazi-Arsa Bedeli Açıklama', 'Metrekaresi', 'Birim Fiyatı TL', 'ARAZİ ARSA BEDELİ', 'Bina İnşaat Gideri Açıklama', 'Ana Bina ve Tesisleri', 'Yardımcı İş. Bina ve Tesisleri', 'İdare Binaları', 'TOPLAM BİNA İNŞAAT GİDERİ', 'Yardımcı İşl. Mak. Teç. Gid.', 'İthalat ve Güm.Giderleri', 'Taşıma ve Sigorta Giderleri', 'Montaj Giderleri', 'Etüd ve Proje Giderleri', 'Diğer Giderleri', 'TOPLAM DİĞER YATIRIM HARCAMALARI', 'TOPLAM SABİT YATIRIM TUTARI TL', 'İthal', 'Yerli', 'Toplam Makine Teçhizat', 'Yeni Makine', 'Kullanılmış Makine', 'TOPLAM İTHAL MAKİNE ($)', 'Toplam Yabancı Kaynak', 'Öz kaynak', 'TOPLAM FİNANSMAN', 'REVIZE TARIHI'
    ];

    // 🎨 ALT KATEGORİ RENK FONKSIYONU - Ana rengtin açık tonu
    const getSubCategoryColor = (categoryName) => {
      const subColors = {
        'KÜNYE BİLGLERİ': 'FFE9D5FF',        // Açık mor
        'YATIRIM İLE İLGİLİ BİLGİLER': 'FFE0F2FE', // Açık mavi  
        'İSTİHDAM': 'FFD1FAE5',              // Açık yeşil
        'ÜRÜN BİLGLERİ': 'FFFEF3C7',        // Açık turuncu
        'DESTEK UNSURLARI': 'FFFECACA',      // Açık kırmızı
        'ÖZEL ŞARTLAR': 'FFE0E7FF',          // Açık indigo
        'FİNANSAL BİLGLER': 'FFECFDF5'       // Açık yeşil
      };
      return subColors[categoryName] || 'FFF8F9FA';
    };

    const totalColumns = columnHeaders.length; // 170+ kolon

    // 🎨 SATIR 1: HER KATEGORİ İÇİN FARKLI RENK SİSTEMİ - ORTALANMİŞ TASARIM
    let currentColumn = 1;
    const categoryMappings = [
      { name: 'KÜNYE BİLGLERİ', columns: 17, startOffset: 0 }, // Biraz daha ortalı
      { name: 'YATIRIM İLE İLGİLİ BİLGİLER', columns: 17, startOffset: 0 },
      { name: 'İSTİHDAM', columns: 2, startOffset: 0 },
      { name: 'ÜRÜN BİLGLERİ', columns: 54, startOffset: 0 },
      { name: 'DESTEK UNSURLARI', columns: 16, startOffset: 0 },
      { name: 'ÖZEL ŞARTLAR', columns: 28, startOffset: 0 },
      { name: 'FİNANSAL BİLGLER', columns: 27, startOffset: 0 }
    ];

    // Her kategori için ayrı renk uygula
    categoryMappings.forEach(category => {
      // İlk sütunda kategori adını yaz
      const cell = mainSheet.getCell(1, currentColumn);
      cell.value = category.name;
      cell.style = categoryStyles[category.name];
      
      // Kategorinin tüm sütunlarını merge et
      if (category.columns > 1) {
        mainSheet.mergeCells(1, currentColumn, 1, currentColumn + category.columns - 1);
      }
      
      // Diğer sütunları boş bırak ama style uygula
      for (let i = 1; i < category.columns; i++) {
        const nextCell = mainSheet.getCell(1, currentColumn + i);
        nextCell.style = categoryStyles[category.name];
      }
      
      currentColumn += category.columns;
    });

    // 🎨 SATIR 2: ALT KATEGORİLER - HER KATEGORİ AYNI RENK TONUNDA
    currentColumn = 1;
    categoryMappings.forEach(category => {
      // Alt kategori rengini ana kategorinin açık tonu yap
      const lightStyle = {
        font: { bold: true, color: { argb: 'FF1F2937' }, size: 10, name: 'Calibri' },
        fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: getSubCategoryColor(category.name) } },
        alignment: { horizontal: 'center', vertical: 'middle', wrapText: true },
        border: { 
      ...
      for (let i = 0; i < category.columns; i++) {
        const cell = mainSheet.getCell(2, currentColumn + i);
        cell.value = subCategories[currentColumn + i - 1] || '';
        cell.style = lightStyle;
      }
      
      currentColumn += category.columns;
    });

    // 🎨 SATIR 3: Özel kategoriler
    for (let i = 0; i < totalColumns; i++) {
      const cell = mainSheet.getCell(3, i + 1);
      cell.value = specialCategories[i] || '';
      cell.style = specialCategoryStyle;
    }

    // 🎨 SATIR 4: Kolon başlıkları
    for (let i = 0; i < totalColumns; i++) {
      const cell = mainSheet.getCell(4, i + 1);
    ...
    // 📊 SATIR 5: Gerçek veri satırı - CSV'deki tam format (170+ kolon)
    const dataRow = [];
    
    // KÜNYE BİLGİLERİ (6 kolon)
    dataRow.push(
      tesvik.gmId || tesvik.tesvikId || '',                    // GM ID
      tesvik.durumBilgileri?.genelDurum || '',                // TALEP/SONUÇ
      tesvik.revizyonlar?.length || 0,                        // REVIZE ID
      tesvik.firma?.firmaId || '',                            // FIRMA ID
      tesvik.yatirimciUnvan || tesvik.firma?.tamUnvan || '',  // YATIRIMCI UNVAN
      tesvik.firma?.sgkSicilNo || ''                          // SGK SİCİL NO
    );
    
    // BELGE BİLGLERİ (11 kolon)
    dataRow.push(
      tesvik._id?.toString() || '',                           // BELGE ID
      tesvik.belgeYonetimi?.belgeNo || '',                    // BELGE NO
      formatDate(tesvik.belgeYonetimi?.belgeTarihi),          // BELGE TARIHI
      formatDate(tesvik.belgeYonetimi?.muracaatTarihi),       // BELGE MURACAAT TARIHI
      tesvik.belgeYonetimi?.muracaatSayisi || '',             // MÜRACAAT SAYISI
      formatDate(tesvik.belgeYonetimi?.belgeBaslamaTarihi),   // BELGE BASLAMA TARIHI
      formatDate(tesvik.belgeYonetimi?.belgeBitisTarihi),     // BELGE BITIS TARIHI
      formatDate(tesvik.belgeYonetimi?.sureUzatimTarihi),     // SÜRE UZATIM TARİHİ
      tesvik.belgeYonetimi?.ozellikliYatirim || '',           // ÖZELLİKLİ YATIRIM İSE
      tesvik.belgeYonetimi?.dayandigiKanun || '',             // DAYANDIĞI KANUN
      tesvik.durumBilgileri?.genelDurum || ''                 // BELGE DURUMU
    );
    
    // YATIRIM İLE İLGİLİ BİLGİLER (17 kolon)
    dataRow.push(
      tesvik.yatirimBilgileri?.yatirimKonusu || '',           // 2-YATIRIM KONUSU
      tesvik.yatirimBilgileri?.yatirimCinsi?.[0] || '',       // 3-CINSI(1)
      tesvik.yatirimBilgileri?.yatirimCinsi?.[1] || '',       // 3-CINSI(2)
      tesvik.yatirimBilgileri?.yatirimCinsi?.[2] || '',       // 3-CINSI(3)
      tesvik.yatirimBilgileri?.yatirimCinsi?.[3] || '',       // 3-CINSI(4)
      tesvik.yatirimBilgileri?.destekSinifi || '',            // DESTEK SINIFI
      tesvik.yatirimBilgileri?.yerinIl || '',                 // YERI IL
      tesvik.yatirimBilgileri?.yerinIlce || '',               // YERI ILCE
      tesvik.yatirimBilgileri?.ada || '',                     // ADA
      tesvik.yatirimBilgileri?.parsel || '',                  // PARSEL
      tesvik.yatirimBilgileri?.yatirimAdresi || '',           // YATIRIM ADRESI(1)
      '',                                                     // YATIRIM ADRESI(2)
      '',                                                     // YATIRIM ADRESI(3)
      tesvik.yatirimBilgileri?.osbMudurluk || '',             // OSB ISE MUDURLUK
      tesvik.yatirimBilgileri?.ilBazliBolge || '',            // İL BAZLI BÖLGE
      tesvik.yatirimBilgileri?.ilceBazliBolge || '',          // İLÇE BAZLI BÖLGE
      tesvik.yatirimBilgileri?.serbestBolge || ''             // SERBEST BÖLGE
    );
    
    // İSTİHDAM (2 kolon)
    dataRow.push(
      tesvik.istihdam?.mevcutKisi || 0,                       // Mevcut Kişi
      tesvik.istihdam?.ilaveKisi || 0                         // İlave Kişi
    );
    
    // ÜRÜN BİLGİLERİ (54 kolon - 9 ürün x 6 alan)
    for (let i = 0; i < 9; i++) {
      const urun = tesvik.urunler?.[i];
      dataRow.push(
        urun?.us97Kodu || '',           // US97 Kodu
        urun?.urunAdi || '',            // Ürün Adı
        urun?.mevcutKapasite || 0,      // Mevcut
        urun?.ilaveKapasite || 0,       // İlave
        urun?.toplamKapasite || 0,      // Toplam
        urun?.kapasiteBirimi || ''      // Kapasite Birimi
      );
    }
    
    // DESTEK UNSURLARI (16 kolon - 8 destek x 2 alan)
    for (let i = 0; i < 8; i++) {
      const destek = tesvik.destekUnsurları?.[i];
      dataRow.push(
        destek?.destekTuru || '',       // Destek Unsuru
        destek?.sartlar || ''           // Şartları
      );
    }
    
    // ÖZEL ŞARTLAR (28 kolon - 14 şart x 2 alan)
    for (let i = 0; i < 14; i++) {
      const ozelSart = tesvik.ozelSartlar?.[i];
      dataRow.push(
        ozelSart?.kisaltma || '',       // Özel Şart Kısaltma
        ozelSart?.aciklama || ''        // Özel Şart Notu
      );
    }
    
    // FİNANSAL BİLGİLER (27 kolon)
    const mali = tesvik.maliHesaplamalar || {};
    dataRow.push(
      mali.araziArsa?.aciklama || '',                         // Arazi-Arsa Bedeli Açıklama
      mali.araziArsa?.metrekare || 0,                         // Metrekaresi
      mali.araziArsa?.birimFiyat || 0,                        // Birim Fiyatı TL
      mali.araziArsa?.toplam || 0,                            // ARAZİ ARSA BEDELİ
      mali.binaInsaat?.aciklama || '',                        // Bina İnşaat Gideri Açıklama
      mali.binaInsaat?.anaBina || 0,                          // Ana Bina ve Tesisleri
      mali.binaInsaat?.yardimciBina || 0,                     // Yardımcı İş. Bina ve Tesisleri
      mali.binaInsaat?.idareBina || 0,                        // İdare Binaları
      mali.binaInsaat?.toplam || 0,                           // TOPLAM BİNA İNŞAAT GİDERİ
      mali.digerYatirim?.yardimciMakine || 0,                 // Yardımcı İşl. Mak. Teç. Gid.
      mali.digerYatirim?.ithalatGiderleri || 0,               // İthalat ve Güm.Giderleri
      mali.digerYatirim?.tasimaGiderleri || 0,                // Taşıma ve Sigorta Giderleri
      mali.digerYatirim?.montajGiderleri || 0,                // Montaj Giderleri
      mali.digerYatirim?.etudProje || 0,                      // Etüd ve Proje Giderleri
      mali.digerYatirim?.digerGiderler || 0,                  // Diğer Giderleri
      mali.digerYatirim?.toplam || 0,                         // TOPLAM DİĞER YATIRIM HARCAMALARI
      mali.toplamSabitYatirim || 0,                           // TOPLAM SABİT YATIRIM TUTARI TL
      mali.makineTeçhizat?.ithalMakine || 0,                  // İthal
      mali.makineTeçhizat?.yerliMakine || 0,                  // Yerli
      mali.makineTeçhizat?.toplamMakine || 0,                 // Toplam Makine Teçhizat
      mali.makineTeçhizat?.yeniMakine || 0,                   // Yeni Makine
      mali.makineTeçhizat?.kullanimisMakine || 0,             // Kullanılmış Makine
      mali.ithalMakineDolar?.toplam || 0,                     // TOPLAM İTHAL MAKİNE ($)
      mali.finansman?.yabanciKaynak || 0,                     // Toplam Yabancı Kaynak
      mali.finansman?.ozKaynak || 0,                          // Öz kaynak
      mali.finansman?.toplam || 0,                            // TOPLAM FİNANSMAN
      formatDateTime(tesvik.updatedAt)                        // REVIZE TARIHI
    );
    
    // 🎨 SATIR 5: Veri satırını excel'e ekle - FİNANSAL VERİ VURGULU
    for (let i = 0; i < dataRow.length; i++) {
      const cell = mainSheet.getCell(5, i + 1);
      cell.value = dataRow[i];
      
      // 💰 FİNANSAL SÜTUNLARI TESPIT ET (CSV'deki finansal bölümler)
      const isFinancialColumn = (
        (i >= 36 && i <= 42) ||    // ARAZİ-ARSA BEDELİ bölümü (sütun 37-43)
        (i >= 43 && i <= 51) ||    // BİNA İNŞAAT GİDERLERİ TL (sütun 44-52)
        (i >= 52 && i <= 58) ||    // DİĞER YATIRIM GİDERLERİ TL (sütun 53-59)
        (i >= 59 && i <= 67) ||    // MAKİNE-EKİPMAN GİDERLERİ TL (sütun 60-68)
        (i >= 68 && i <= 76) ||    // İTHAL MAKİNE-EKİPMAN GİDERLERİ TL (sütun 69-77)
        (i >= 77 && i <= 85) ||    // TOPLAM YATIRIM (sütun 78-86)
        (i >= 86 && i <= 92)       // FİNANSMAN KAYNAKLARI (sütun 87-93)
      );
      
      // 💎 FİNANSAL VE NORMAL VERİ STİLİNİ UYGULA
      if (isFinancialColumn && typeof dataRow[i] === 'number' && dataRow[i] > 0) {
        cell.style = financialDataStyle;
        // Para formatı için özel değer kontrolü
        if (dataRow[i] && !isNaN(dataRow[i])) {
          cell.value = Number(dataRow[i]);
        }
      } else {
        cell.style = dataStyle;
      }
    ...
    // 🌈 ALTERNATING ROW COLORS İÇİN TEMELİ AT (İleride çoklu veri satırı olursa)
    const alternateRowStyle = {
      ...dataStyle,
      fill: { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFBFBFB' } } // Çok açık gri
    };
    
    // 🏢 PROFESYONEL SAYFA DÜZENİ VE FORMAT AYARLARI
    
    // 🎨 ULTRA PROFESYONEL LAYOUT DÜZENLEMELERİ
    
    // 📏 SÜTUN GENİŞLİKLERİ - Kategori bazlı optimizasyon
    for (let i = 1; i <= totalColumns; i++) {
      if (i <= 6) {                    // KÜNYE - Geniş alanlar
        mainSheet.getColumn(i).width = 18;
      } else if (i <= 23) {            // YATIRIM & İSTIHDAM - Orta genişlik
        mainSheet.getColumn(i).width = 14;
      } else if (i <= 77) {            // ÜRÜN BİLGLERİ - Küçük genişlik
        mainSheet.getColumn(i).width = 11;
      } else if (i <= 93) {            // DESTEK USURLARI - Orta genişlik
        mainSheet.getColumn(i).width = 13;
      } else if (i <= 121) {           // ÖZEL ŞARTLAR - Geniş alan
        mainSheet.getColumn(i).width = 15;
      } else {                         // FİNANSAL BİLGİLER - En geniş
        mainSheet.getColumn(i).width = 16;
      }
    }
    
    // 📐 SATIR YÜKSEKLİKLERİ - Profesyonel hiyerarşi
    mainSheet.getRow(1).height = 35;   // Ana kategoriler - En yüksek
    mainSheet.getRow(2).height = 28;   // Alt kategoriler - Orta yüksek  
    mainSheet.getRow(3).height = 22;   // Özel kategoriler - Orta
    mainSheet.getRow(4).height = 20;   // Kolon başlıkları - Normal
    mainSheet.getRow(5).height = 18;   // Veri satırı - Kompakt
    
    // ❄️ FROZEN PANES - İlk 4 satır ve 3 sütun donuk kalsın
    mainSheet.views = [
      {
        state: 'frozen',
        xSplit: 3,      // İlk 3 sütun donuk
        ySplit: 4,      // İlk 4 satır donuk
        topLeftCell: 'D5',
        activeCell: 'D5',
        showGridLines: true,
    ...
    // 🏗️ WORKSHEET AYARLARI - Professional defaults
    mainSheet.properties.defaultRowHeight = 16;
    mainSheet.properties.defaultColWidth = 12;
    
    // 💎 ELİT STRATEJİK SÜTUN GENİŞLİKLERİ - Kurumsal Standart
    mainSheet.getColumn(1).width = 15;   // GM ID - Kimlik net
    mainSheet.getColumn(2).width = 15;   // TALEP/SONUÇ - Durum belirgin
    mainSheet.getColumn(3).width = 15;   // VERGİ NO/TC - Kimlik
    mainSheet.getColumn(4).width = 15;   // FIRMA ID - Referans
    mainSheet.getColumn(5).width = 32;   // YATIRIMCI UNVAN - Tam görünüm
    mainSheet.getColumn(6).width = 15;   // SGK SİCİL NO - Kimlik
    mainSheet.getColumn(7).width = 18;   // BELGE NO - Önemli referans
    mainSheet.getColumn(8).width = 15;   // BELGE TARİHİ - Tarih kompakt
    mainSheet.getColumn(9).width = 15;   // VİZE TARİHİ - Tarih
    mainSheet.getColumn(10).width = 15;  // BAŞLAMA TARİHİ - Tarih
    mainSheet.getColumn(11).width = 15;  // BİTİS TARİHİ - Tarih
    mainSheet.getColumn(12).width = 15;  // VİZE BİTİS TARİHİ - Tarih
    mainSheet.getColumn(13).width = 15;  // İPTAL TARİHİ - Tarih
    mainSheet.getColumn(18).width = 35;  // YATIRIM KONUSU - Detaylı
    mainSheet.getColumn(23).width = 18;  // YERI IL - Coğrafi
    mainSheet.getColumn(24).width = 18;  // YERI ILCE - Coğrafi
    
    // 💰 FİNANSAL SÜTUNLAR İÇİN ÖZEL GENİŞLİK (Para gösterimi için)
    for (let i = 37; i <= 93; i++) {     // Tüm finansal sütunlar
      mainSheet.getColumn(i).width = 14; // Para gösterimi için optimal
    }
    
    // 🏆 ELİT SATIR YÜKSEKLİKLERİ - Mükemmel Görsel Hiyerarşi
    mainSheet.getRow(1).height = 28;     // Ana kategoriler - Executive yükseklik
    mainSheet.getRow(2).height = 24;     // Alt kategoriler - Premium yükseklik  
    mainSheet.getRow(3).height = 20;     // Özel kategoriler - Profesyonel yükseklik
    mainSheet.getRow(4).height = 16;     // Kolon başlıkları - Kompakt yükseklik
    mainSheet.getRow(5).height = 15;     // Veri satırı - Optimize edilmiş yükseklik
    
    // 🎯 ULTRA PRO WORKSHEET AYARLARI
    mainSheet.views = [{
      state: 'frozen', 
      xSplit: 0, 
      ySplit: 4,              // İlk 4 satırı dondur (başlıklar)
      topLeftCell: 'A5',      // Kaydırma A5'ten başlasın
      activeCell: 'A5',
      showGridLines: true,    // Grid çizgileri görünür
      showRowColHeaders: true // Satır/sütun başlıkları görünür
    }];
    
    // 🎨 ELİT SHEET DÜZENİ
    mainSheet.properties.defaultRowHeight = 15;  // Optimal varsayılan yükseklik
    mainSheet.properties.defaultColWidth = 12;   // Daha kompakt genişlik
    mainSheet.properties.outlineLevelCol = 0;    // Outline seviyesi
    mainSheet.properties.outlineLevelRow = 0;
    
    // 📋 WORKBOOK META BİLGİLERİ - Profesyonel Kimlik (ExcelJS Formatı)
    workbook.creator = 'Belge Teşvik Sistemi';
    workbook.lastModifiedBy = 'Sistem Excel Modülü';
    workbook.created = new Date();
    workbook.modified = new Date();
    workbook.title = `${tesvik.firma?.tamUnvan || 'Firma'} - Sistem Excel Raporu`;
    workbook.subject = 'Teşvik Detay Raporu';
    workbook.keywords = 'teşvik,rapor,sistem,excel,detay';
    workbook.category = 'Resmi Belgeler';
    workbook.description = `${tesvik.tesvikId || tesvik.gmId} nolu teşvik için detaylı sistem raporu`;
    
    // 📂 PROFESYONEL DOSYA ADI OLUŞTUR
    const currentDate = new Date().toISOString().split('T')[0].replace(/-/g, '');
    const firmaKisaAd = (tesvik.firma?.tamUnvan || 'FIRMA').substring(0, 15).toUpperCase().replace(/[^A-Z0-9]/g, '');
    const tesvikNo = tesvik.tesvikId || tesvik.gmId || 'TSV';
    const fileName = `SISTEM_EXCEL_RAPORU_${firmaKisaAd}_${tesvikNo}_${currentDate}.xlsx`;
    ...
    console.log(`✅ Sistem Excel çıktısı oluşturuldu: ${fileName} (${buffer.length} bytes)`);
    ...
    res.status(500).json({
      success: false,
      message: 'Sistem Excel çıktısı oluşturulurken hata oluştu',
      error: error.message
    });
  }
};