...
router.get('/bulk-excel-export', authenticate, checkPermission('raporGoruntule'), async (req, res) => {
  try {
    ...
    // Filtrelenmiş teşvikleri getir (Firma modelindeki gerçek alan adları: tamUnvan/firmaIl)
    ...
    tesvikler.forEach((tesvik, index) => {
      ...
      const rowData = [
        ...
        tesvik.yatirimBilgileri?.yerinIl || tesvik.firma?.firmaIl || '',
        tesvik.yatirimBilgileri?.yerinIlce || '',
